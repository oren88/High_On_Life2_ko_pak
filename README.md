...
이 한글패치는 개인 취미용으로 제작된 비공식 패치이며, AI 보조 번역을 포함하고 있어 일부 미번역, 오역, 어색한 표현, 표시 문제가 남아 있을 수 있습니다.
...
- Windows용 Xbox 버전 기준
- Steam 버전은 아직 테스트하지 않았으므로, 정상 동작을 보장하지 않습니다.
...
5. 게임을 실행한 뒤 텍스트 언어를 확인합니다.

## 중요

- 현재는 게임 내 텍스트 언어를 `한국어` 또는 `기본값`으로 두어도 한글이 정상 표시되는 환경이 확인되었습니다.
- 따라서 먼저 `한국어` 또는 `기본값` 상태에서 패치가 잘 보이는지 확인하는 것을 권장합니다.
- 환경에 따라 언어 선택이 다르게 동작할 수 있으므로, 만약 한글이 보이지 않으면 그때 `zh` 슬롯도 함께 확인해보면 됩니다.
- `한국어`, `기본값`, `zh` 중 정상적으로 한글이 표시되는 설정을 그대로 사용하면 됩니다.
...
- Steam 버전은 테스트하지 않았으므로 적용되지 않거나 예상과 다르게 동작할 수 있습니다.
...
- 이미지 에셋에 직접 포함된 텍스트는 한글로 바뀌지 않아 영어가 남아 있을 수 있습니다.
